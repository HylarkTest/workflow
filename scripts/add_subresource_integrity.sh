...
for URL in "${URLS[@]}" ; do
    HASH=$(curl -s -H "Origin: https://app.hylark.com" "$URL" | openssl dgst -sha384 -binary | openssl base64 -A)
    ...
    for FILE in "${FILES[@]}" ; do
        ...
        $SED_COMMAND -i -E "s~<script src=\"$URL\"( integrity=\"[^\"]+\")?~<script src=\"$URL\" integrity=\"sha384-$HASH\"~" "$FILE"
    done
done
