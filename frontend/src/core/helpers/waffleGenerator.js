...
function retrieveRandomWordOfType(type) {
    ...
    return vocab[type][_.random(vocab[type].length - 1)];
}

function getRandomVocabType() {
    const types = _.keys(vocab);
    return types[_.random(types.length - 1)];
}

export function generateWord() {
    return retrieveRandomWordOfType(getRandomVocabType());
}

export function generateSentence(topic = null) {
    if (!topic || !_.has(sentencePatterns, topic)) {
        const topics = _.keys(sentencePatterns);
        // eslint-disable-next-line no-param-reassign
        topic = topics[_.random(topics.length - 1)];
    }

    const patternNumber = _.random(sentencePatterns[topic].length - 1);
    let pattern = sentencePatterns[topic][patternNumber];
    const matchPattern = new RegExp(`(${_.keys(vocab).join('|')})`, 'ig');
    pattern = pattern.replaceAll(
        matchPattern,
        (match) => {
            const word = retrieveRandomWordOfType(match);
            if (word) {
                return word;
            }
            return match;
    ...
    return _.capitalize(pattern);
}

export function generateSentences(count = 3, topic = null) {
    const sentences = [];
    for (let i = 0; i < count; i += 1) {
        sentences.push(generateSentence(topic));
    }
    return sentences;
}

export function generateParagraph(sentenceCount = 3, topic = null) {
    const paragraph = [];
    for (let i = 0; i < sentenceCount; i += 1) {
        const sentence = generateSentence(topic);
        paragraph.push(sentence);
    }
    return paragraph.join(' ');
}

export function generateParagraphs(count = 3, topic = null) {
    const paragraphs = [];
    for (let i = 0; i < count; i += 1) {
        const sentenceCount = _.random(2, 5);
        paragraphs.push(generateParagraph(sentenceCount, topic));
    }
    return paragraphs;
}

export function generateText(characterCount = 200) {
    let text = '';
